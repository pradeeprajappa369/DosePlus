interface SubscriptionDistributionChartProps {
  data: Array<{ plan: string; count: number; percentage: number; color: string }>;
}

export default function SubscriptionDistributionChart({ data }: SubscriptionDistributionChartProps) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
      <div className="mb-6">
        <h3 className="text-lg font-bold text-gray-900">Subscription Distribution</h3>
        <p className="text-sm text-gray-500 mt-1">Active plans breakdown</p>
      </div>
        ...
        <div className="space-y-4">
          {data.map((item, index) => (
            <div key={index} className="flex items-center gap-3">
              ...
              ></div>
              <div>
                <p className="text-sm font-semibold text-gray-900">{item.plan}</p>
